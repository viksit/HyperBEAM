...
%% WASM files
-define(WASM_MINIMAL, "test/fast_jit/minimal/32_68.wasm").
-define(WASM_CORE, "test/fast_jit/core.wasm").
-define(WASM_SQL, "test/64/aos-sql.wasm").
...
minimal_test() ->
    WasmFile = ?WASM_MINIMAL,
    WasmBinary = setup_env(WasmFile),
    ?event("Running Minimal Fast JIT Test"),
    {ok, Port, _Imports, _Exports} = hb_beamr:start(WasmBinary),
    Env = gen_test_env(),
    Msg = gen_test_aos_msg("fast jit test"),
    ...
    case parse_wasm_response(ResponseBin) of
        {ok, Response} ->
            ...
            ?assertMatch(<<"fast jit test">>, Response);
        {error, Reason} ->
    ...
    %% Stop the WASM
    hb_beamr:stop(Port),
    ?event("Minimal Fast JIT Test passed").

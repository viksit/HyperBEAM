...
path(Opts, [Part1, Part2, LookupKeys]) ->
	...
	% so we want to make a Binary from Part1, Part2 and perform
	% a lookup based on given lookup keys.
	Subpath = path(Opts, [Part1, Part2]),
	case is_charlist(LookupKeys) of
		true ->
			% returns regular binary path
			path(Opts, [Subpath, LookupKeys]);
		false ->
			% returns list with path and lookup keys
			[Subpath, LookupKeys]
	end;
